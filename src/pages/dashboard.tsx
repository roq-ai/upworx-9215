...
import { useRoqPlatformUserProfiles, useJobApplicationCount, useJobOfferCount, useContractCount } from 'lib/roq';

import { ContractListPage as ShowCaseList } from 'pages/contracts';

function DashboardPage() {
  const { session } = useSession();

  const jobApplicationStat = useJobApplicationCount();
  ...
  const statResults: { title: string; result: number; change: number; isLoading: boolean }[] = [
    {
      ...
      result: jobApplicationStat?.data,
      isLoading: jobApplicationStat.isLoading,
          ...
          <Stack direction="column" spacing={8} flex={{ base: 1, md: 7 }}>
            ...
            <Box>
              <Flex mb={4} justifyContent={'space-between'}>
                ...
                <NextLink href={`contracts`} passHref legacyBehavior>
                  <Button

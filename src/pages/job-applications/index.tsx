...
import { useRoqClient, useJobApplicationFindManyWithCount } from 'lib/roq';
import { convertQueryToPrismaUtil } from 'lib/utils';

type ColumnType = ColumnDef<JobApplicationInterface, unknown>;

interface JobApplicationListPageProps {
...
export function JobApplicationListPage(props: JobApplicationListPageProps) {
  const {
    filters = {},
    titleProps = {},
      ...
      convertQueryToPrismaUtil(
        {
          ...
          searchTermKeys: ['application_status.contains', 'application_feedback.contains'],
          ...params.filters,
        },
        'job_application',
      ),
  ...
  const { data, error, isLoading, mutate } = useJobApplicationFindManyWithCount({
  ...
  const handleDelete = async (id: string) => {
    setDeleteError(null);
    try {
      await roqClient.job_application.delete({ where: { id } });
      await mutate();
    } catch (error) {
  ...
  const handleView = (row: JobApplicationInterface) => {
    if (hasAccess('job_application', AccessOperationEnum.READ, AccessServiceEnum.PROJECT)) {
      router.push(`/job-applications/view/${row.id}`);
    }
  };

  const columns: ColumnType[] = [
    { id: 'application_status', header: 'Application Status', accessorKey: 'application_status' },
    { id: 'application_feedback', header: 'Application Feedback', accessorKey: 'application_feedback' },
    ...
    <Flex direction="column" gap={{ md: 6, base: 7 }} shadow="none">
      <Flex justifyContent={{ md: 'space-between' }} direction={{ base: 'column', md: 'row' }} gap={{ base: '28px' }}>
        <Flex alignItems="center" gap={1}>
          ...
          <AccessInfo entity="job_application" />
        </Flex>

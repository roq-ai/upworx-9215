...
import { useRoqClient, useJobApplicationFindFirst } from 'lib/roq';
...
function JobApplicationEditPage() {
  ...
  const { data, error, isLoading, mutate } = useJobApplicationFindFirst(queryParams, {}, { disabled: !id });
  const [formError, setFormError] = useState(null);

  const handleSubmit = async (values: JobApplicationInterface, { resetForm }: FormikHelpers<any>) => {
    setFormError(null);
    try {
      const updated = await roqClient.job_application.update({
        data: values as RoqTypes.job_application,
        where: {
          id,
        },
      });
